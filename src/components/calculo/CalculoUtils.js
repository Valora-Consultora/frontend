...
const obtenerFactorConservacion = (estado) => {
    return FACTORES_CONSERVACION[estado] || 1.00;
};
...
const obtenerEstadoConservacionDesdeValor = (factor) => {
    ...
    if (factor === 0.90) return "Necesita Mantenimiento";
...
const calcularFactorEdad = (promedioEdad) => {
    ...
    const factorEdadCalculado = 1 - 0.5 * ((promedioEdad * promedioEdad) + promedioEdad);
 ...
 */
const calcularPrecioMetroCorregido = (precioMetro, factorEdad, factorConservacion) => {
    if (!precioMetro || !factorEdad || !factorConservacion) return 0;
    
    const valorCorregido = parseFloat(precioMetro) * parseFloat(factorConservacion) * parseFloat(factorEdad);
...
/**
 * Calcula el valor intrínseco (suma del valor del terreno y la obra civil)
 ...
 */
const calcularValorIntrinseco = (valorTerreno, valorObraCivil) => {
...
const calcularValorReposicionNuevo = (superficies) => {
    ...
    const total = superficies.reduce((total, superficie) => {
        ...
        return total + (parseFloat(superficie.precioMetro || 0) * parseFloat(superficie.m2 || 0));
    }, 0);
...
const calcularValorTerreno = (superficieTerreno, valorMetroTerreno) => {
    return formatearNumero(parseFloat(superficieTerreno) * parseFloat(valorMetroTerreno));
};
 ...
 */
const calcularValorVentaRapida = (valorMercado) => formatearNumero(parseFloat(valorMercado) * 0.90);
 ...
 */
const calcularValorRemate = (valorMercado) => formatearNumero(parseFloat(valorMercado) * 0.80);
 ...
 */
const verificarDiferenciaHomologacion = (valorCalculado, valorHomologado, porcentajeMaximo = 20) => {
    if (!valorCalculado || !valorHomologado) return false;
    
    const diferenciaPorcentual = Math.abs((valorHomologado - valorCalculado) / valorCalculado * 100);
...
const recalcularSuperficie = (superficie) => {
    ...
    resultado.conservacion = obtenerEstadoConservacionDesdeValor(factorConservacion);
